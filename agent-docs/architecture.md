...
- **Данные (личные vs ролевые):**
  ...
  - **`ClientAccountProfile`** — 1:1 с `User`, только для роли `CLIENT`: предпочтения кабинета (часовой пояс, язык, обращение, флаги уведомлений и т.д.). Строка создаётся лениво при первом `PATCH`. Новые поля — отдельными колонками в Prisma + валидация в [`app/api/user/profile/route.ts`](../app/api/user/profile/route.ts).
- **API:** `GET`/`PATCH` [`/api/user/profile`](../app/api/user/profile/route.ts); тип ответа — [`lib/user-settings/types.ts`](../lib/user-settings/types.ts) (`UserSettingsProfile`: `user`, `psychologistProfile` | `null`, `clientAccountProfile` | `null`). У психолога `clientAccountProfile` всегда `null`. PATCH: вложенный объект `clientAccountProfile` обрабатывается только при `role === CLIENT`.
- **Клиентский UI:**
  ...
  - Вкладка «Предпочтения»: [`hooks/use-client-account-preferences-tab-ui.ts`](../hooks/use-client-account-preferences-tab-ui.ts) + [`components/client/settings/client-account-preferences-tab.tsx`](../components/client/settings/client-account-preferences-tab.tsx) (`value="preferences"` в [`client-settings-tabs-list.tsx`](../components/settings/shared/client-settings-tabs-list.tsx)).
  - Общая оболочка вкладок: [`components/settings/shared/settings-form-tabs-layout.tsx`](../components/settings/shared/settings-form-tabs-layout.tsx) — три базовые вкладки (`profile`, `security`, `accounts`), проп `profileTabFooter` для доп. блоков на вкладке «Личные данные», `children` — дополнительные `TabsContent` (клиент: `preferences`; психолог: профессиональный профиль, календарь, поля клиента, статусы).
  - Клиент: [`components/client/settings/client-settings-form.tsx`](../components/client/settings/client-settings-form.tsx).
...
- **Плановые улучшения (не блокеры):** по желанию объединить `useClientSettings`/`useProfileSettings`; вынести общую секцию «Личные данные» (`SettingsSection` + `PersonalProfileForm`), если разметка клиента и психолога снова разойдётся; подключить `locale`/`marketingEmailsOptIn` к i18n и рассылкам.
