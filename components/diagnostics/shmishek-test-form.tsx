...
const VALID_ANSWER_VALUES = [0, 1] as const;
...
export function ShmishekTestForm({ token, questions }: Props) {
  ...
  const { submit, submitting, submitError, result } = useDiagnosticSubmit(
    token,
    "/api/diagnostics/shmishek/submit",
    markAsSubmitted
  );
  ...
  const getAnswer = (questionIndex: number): AnswerValue | undefined => {
    ...
    if (v === 0 || v === 1) return v as AnswerValue;
    return undefined;
  };
      ...
      <Card className="relative max-w-2xl w-full mx-auto">
        <CardHeader className="pr-10">
          <CardTitle className="text-lg">
            Результаты: Тест по изучению акцентуаций характера (детский вариант) Шмишек Х.
          </CardTitle>
        </CardHeader>
            ...
            {(() => {
              ...
              return (
                <>
                  ...
                  <p className="text-xs text-muted-foreground">Выберите ответ: Да или Нет</p>
